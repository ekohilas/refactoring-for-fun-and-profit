...
const Plugin = () => {
	...
	function createMarkdownSlide( content, options ) {
		...
		if (content.includes("```\n")) {
		    let postFenceIndex = content.lastIndexOf("```") + 3;
		    fencedContent = content.slice(0, postFenceIndex + 1);
		    newContent = content.slice(postFenceIndex);
		}

		var notesMatch = newContent.split( new RegExp( options.notesSeparator, 'mgi' ));

		if( notesMatch.length >= 2 ) {
			var notes = notesMatch.slice(1).join("\n\n").trim(); 
			var contentMatch = notesMatch[0];
			console.log("content %o, notes: %o", contentMatch, notes);
			content = marked(fencedContent + contentMatch) + '<aside class="notes">' + marked(notes) + '</aside>';
		}
